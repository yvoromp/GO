		...
		try{
			while(in.readLine() != null){
				...
				case READY:
					...
					String status1 = splited[2];
					stoneStatus = status1;
					Status p1 = isBlack(status1);
					String status2 = splited[4];
					Status p2 = isBlack(status2);
					Player player1 = new WebPlayer(splited[3], p1, Integer.parseInt(splited[1]));
					Player player2 = new WebPlayer(splited[5], p2, Integer.parseInt(splited[1]));
					game = new Game(player1,player2,Integer.parseInt(splited[1]));
					game.start();
					break;
				case CHAT:
					print(serverInput);
